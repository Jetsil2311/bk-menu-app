...
export const AdminLayout = () => {
  const { isAuthReady, isAuthenticated, user, handleLogout, email, setEmail, password, setPassword, error, isSubmitting, handleLogin, handleGoogleSignIn } = useAdminAuth()
  ...
  return (
    <div className="min-h-screen bg-main-950 flex">
      <Sidebar />
      <div className="flex-1 flex flex-col min-w-0">
        <Topbar user={user} handleLogout={handleLogout} />
        <main className="flex-1 bg-[#2a1208] rounded-tl-3xl border-t border-l border-white/5 p-6 md:p-8 overflow-y-auto">
          <Outlet />
        </main>
      </div>
    </div>
